    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.side_effect = [
            ...
            subprocess.CalledProcessError(
                1, "git checkout", stderr=b"error: branch not found"
            ),
        ]
...
def test_config_get_release_branch_name_custom():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
        f.write(
            "release-branch-name: my-release-branch\nrelease-rules:\n  patch:\n    - fix\n"
        )
        f.flush()
        config = ReleaseConfig(f.name)
...
"""
    ...
    with (
        ...
        patch(
            "contiamo_release_please.github.create_github_release"
        ...
        assert result["version"] == "v1.2.3"
        assert (
            result["release_url"] == "https://github.com/owner/repo/releases/tag/v1.2.3"
        )
...
"""
    ...
    with (
        ...
        mock_repo_info.return_value = ("owner", "repo")
        mock_create_release.return_value = {
            "html_url": "https://github.com/owner/repo/releases/tag/release-2.5.0"
        }
        ...
        assert (
            result["release_url"]
            == "https://github.com/owner/repo/releases/tag/release-2.5.0"
        )
...
"""
    ...
    with (
        ...
        mock_repo_info.return_value = ("owner", "repo")
        mock_create_release.return_value = {
            "html_url": "https://github.com/owner/repo/releases/tag/3.0.0"
        }
        ...
        assert (
            result["release_url"] == "https://github.com/owner/repo/releases/tag/3.0.0"
        )
...
def test_tag_release_workflow_fails_on_non_release_commit(tmp_path):
    ...
    with (
        ...
        with pytest.raises(
            ReleaseError,
            match="Cannot create release tag: Latest commit is not a release PR merge",
        ):
            tag_release_workflow()
...
def test_tag_release_workflow_succeeds_on_merge_commit(tmp_path):
    ...
    with (
        ...
        mock_branch.return_value = "main"
        mock_commit.return_value = (
            "Merge branch 'release-please--branches--main' into main"  # Merge commit
        )
        mock_tag_exists.return_value = False
        mock_detect_host.return_value = None  # Not GitHub
